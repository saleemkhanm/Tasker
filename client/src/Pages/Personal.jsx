...
export default function Personal() {
    ...
    const fetchCategoryTasks = async () => {
        ...
        try {
            ...
            const res = await fetch(
                `http://localhost:3000/api/v1/tasker/category/6943e47f1cef725c734980ae?userId=${user._id}`
    ...
    return (
        <>
            {user ? (
                <>
                    {user && <SecondNavbar category="Personal" />}

                    {/* CardListing handles empty state */}
                    <CardListing item={items} category="Personal" />
                </>
            ) : (
                <Container
                    fluid
